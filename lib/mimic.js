 ...
 * 
 * @param callback
 *                Optional callback function if you want the request to be asynchronous.
 * 
 * @return XmlRpcResponse object.
 */
XmlRpcRequest.prototype.send = function(callback, timeout) {
	// Vars
	var xml_params = "", 
	    i = 0, 
	    xml_call, xhr;
	...
	if (callback != null) {
		xhr.onreadystatechange = function () {
			callback(new XmlRpcResponse(xhr.responseXML));
		};
	}
	xhr.open("POST", this.serviceUrl, callback != null);
	if (timeout != null) xhr.timeout = timeout;
	xhr.send(Builder.buildDOM(xml_call));
	if (callback == null) return new XmlRpcResponse(xhr.responseXML);
};
